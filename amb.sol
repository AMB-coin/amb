...
contract IDisputeWindow is ITyped, IERC20 {
    ...
    function initialize(Amb _augur, IUniverse _universe, uint256 _disputeWindowId, bool _participationTokensEnabled, uint256 _duration, uint256 _startTime) public;
...
contract IMarket is IOwnable {
    ...
    function initialize(Amb _augur, IUniverse _universe, uint256 _endTime, uint256 _feePerCashInAttoCash, IAffiliateValidator _affiliateValidator, uint256 _affiliateFeeDivisor, address _designatedReporterAddress, address _creator, uint256 _numOutcomes, uint256 _numTicks) public;
...
contract IInitialReporter is IReportingParticipant, IOwnable {
    function initialize(Amb _augur, IMarket _market, address _designatedReporter) public;
...
contract IShareToken is ITyped, IERC1155 {
    function initialize(Amb _augur) external;
...
contract IProfitLoss {
    function initialize(Amb _augur) public;
...
library Order {
    ...
    function create(Amb _augur, IAugurTrading _augurTrading, address _creator, uint256 _outcome, Order.Types _type, uint256 _attoshares, uint256 _price, IMarket _market, bytes32 _betterOrderId, bytes32 _worseOrderId) internal view returns (Data memory) {
...
contract Orders is IOrders, Initializable {
    ...
    function initialize(Amb _augur, IAugurTrading _augurTrading) public beforeInitialized {
        endInitialization();
        cash = ICash(_augur.lookup("Cash"));
